...
from ...adapters.open_to_csv import (
    ExportRequest,
    build_output_name,
    export_open_file,
...
def _save_pipeline30_runtime_settings(payload: Dict[str, Any]) -> None:
    ...
    try:
        save_runtime_state(
            ...
            RuntimeState(
                raw_input_dir=Path(str(current.get("raw_input_dir", "")).strip()).expanduser().resolve()
                if str(current.get("raw_input_dir", "")).strip()
                else None,
                out_dir=Path(str(current.get("out_dir", "")).strip()).expanduser().resolve()
                if str(current.get("out_dir", "")).strip()
                else None,
                selection=RuntimeSelection(
                    aggregation_mode=str(current.get("aggregation_mode", PIPELINE30_SWEEP_DEFAULT_MODE)).strip() or PIPELINE30_SWEEP_DEFAULT_MODE,
                    sweep_key=str(current.get("sweep_key", PIPELINE30_SWEEP_DEFAULT_KEY)).strip() or PIPELINE30_SWEEP_DEFAULT_KEY,
                    sweep_x_col=str(current.get("sweep_x_col", PIPELINE30_SWEEP_DEFAULT_X_COL)).strip() or PIPELINE30_SWEEP_DEFAULT_X_COL,
                    sweep_bin_tol=float(str(current.get("sweep_bin_tol", PIPELINE30_SWEEP_DEFAULT_BIN_TOL)).replace(",", ".")),
                ),
                helper_configured=bool(current.get("helper_configured", False)),
                dirs_configured_in_gui=bool(current.get("dirs_configured_in_gui", False)),
                config_dir=None,
            ),
        )
    except Exception:
...
class Pipeline29ConfigEditor(QMainWindow):
    ...
    def save_preset(self) -> None:
        ...
        if errors:
            QMessageBox.warning(self, "Pipeline 29", "\n".join(errors))
            return
        preset_dir = default_preset_dir(self.base_dir)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    base_dir = Path(args.base_dir).expanduser().resolve() if args.base_dir else Path(__file__).resolve().parents[4]
