...
LOAD_SWEEP_FEATURE_SPECS: tuple[FeatureSpec, ...] = (
    ...
    FeatureSpec(
        ...
        description="Compute delta-T diagnostics and quality summaries, export xlsx (no plots).",
        stage="diagnostics",
        default_by_mode={"load": True, "sweep": True},
        legacy_anchor=legacy_anchor_for_feature("run_time_diagnostics"),
    ),
    FeatureSpec(
        key="plot_time_diagnostics",
        label="Plot time diagnostics",
        description="Generate time-delta PNGs from pre-computed diagnostics data.",
        stage="plotting",
        default_by_mode={"load": True, "sweep": True},
        legacy_anchor="",
        notes="Split from run_time_diagnostics. Requires run_time_diagnostics to populate ctx.time_diagnostics.",
    ),
    FeatureSpec(
    ...
    FeatureSpec(
        key="compute_compare_iteracoes",
        label="Compute compare_iteracoes",
        description="Compute per-metric aggregation, delta tables with GUM uncertainty, and export metricas_incertezas.xlsx.",
        stage="processing",
        default_by_mode={"load": True, "sweep": False},
        legacy_anchor=legacy_anchor_for_feature("run_compare_iteracoes"),
        notes="Processing half of the former run_compare_iteracoes. Populates ctx.compare_iteracoes_* for the plot stage.",
    ),
    FeatureSpec(
        key="plot_compare_iteracoes",
        label="Plot compare_iteracoes",
        description="Generate absolute-value and delta-percentage PNGs from pre-computed compare data.",
        stage="plotting",
        default_by_mode={"load": True, "sweep": False},
        legacy_anchor="",
