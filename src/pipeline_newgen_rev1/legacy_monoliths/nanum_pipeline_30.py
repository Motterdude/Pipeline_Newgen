...
PIPELINE_LABEL = "Pipeline 30"
PIPELINE_RUNTIME_NAME = "pipeline30"

RUNTIME_SETTINGS_DIR = Path(os.environ.get("LOCALAPPDATA", str(Path.home()))) / "nanum_pipeline_30"
RUNTIME_SETTINGS_PATH = RUNTIME_SETTINGS_DIR / "pipeline30_runtime_paths.json"
...
SWEEP_KEY_COL = "Sweep_Key"
SWEEP_DISPLAY_LABEL_COL = "Sweep_Display_Label"
SWEEP_BIN_VALUE_COL = "Sweep_Bin_Value"
SWEEP_BIN_LABEL_COL = "Sweep_Bin_Label"
...
RUNTIME_AGGREGATION_MODE = RUNTIME_AGGREGATION_LOAD
RUNTIME_SWEEP_KEY = DEFAULT_SWEEP_KEY
RUNTIME_SWEEP_X_COL = SWEEP_VALUE_COL
RUNTIME_SWEEP_BIN_TOL = DEFAULT_SWEEP_BIN_TOL
...
@dataclass(frozen=True)
class Pipeline30RuntimeSelection:
...
def _normalize_runtime_aggregation_mode(value: object) -> str:
    text = _to_str_or_empty(value).lower()
...
def _normalize_sweep_key(value: object) -> str:
    text = _to_str_or_empty(value).lower()
    if not text:
...
def _normalize_sweep_x_col(value: object) -> str:
    text = _to_str_or_empty(value)
    return text if text else SWEEP_VALUE_COL


def _normalize_sweep_bin_tol(value: object) -> float:
    tol = _to_float(value, default=DEFAULT_SWEEP_BIN_TOL)
    if not np.isfinite(tol):
        return DEFAULT_SWEEP_BIN_TOL
    return max(float(tol), 0.0)


def _sweep_axis_label(key: object) -> str:
    canonical = _normalize_sweep_key(key)
    return SWEEP_AXIS_LABELS.get(canonical, canonical.upper() if canonical else SWEEP_AXIS_LABELS[DEFAULT_SWEEP_KEY])


def _runtime_sweep_active() -> bool:
    return _normalize_runtime_aggregation_mode(RUNTIME_AGGREGATION_MODE) == RUNTIME_AGGREGATION_SWEEP


def _runtime_sweep_key() -> str:
    return _normalize_sweep_key(RUNTIME_SWEEP_KEY)


def _runtime_sweep_x_col() -> str:
    return _normalize_sweep_x_col(RUNTIME_SWEEP_X_COL)


def _runtime_sweep_bin_tol() -> float:
    return _normalize_sweep_bin_tol(RUNTIME_SWEEP_BIN_TOL)


def _runtime_sweep_effective_x_col() -> str:
    if _runtime_sweep_active():
        return SWEEP_BIN_VALUE_COL
    return _runtime_sweep_x_col()


def _runtime_sweep_matches_load_request(x_col_req: object) -> bool:
    if not _runtime_sweep_active():
        return False
    req = _to_str_or_empty(x_col_req)
    return (not req) or norm_key(req) == norm_key("Load_kW")


def _runtime_set_plot_mode(selection: Pipeline30RuntimeSelection) -> None:
    global RUNTIME_AGGREGATION_MODE, RUNTIME_SWEEP_KEY, RUNTIME_SWEEP_X_COL, RUNTIME_SWEEP_BIN_TOL
    RUNTIME_AGGREGATION_MODE = _normalize_runtime_aggregation_mode(selection.aggregation_mode)
    RUNTIME_SWEEP_KEY = _normalize_sweep_key(selection.sweep_key)
    RUNTIME_SWEEP_X_COL = _normalize_sweep_x_col(selection.sweep_x_col)
    RUNTIME_SWEEP_BIN_TOL = _normalize_sweep_bin_tol(selection.sweep_bin_tol)
...
def _parse_filename_sweep(stem: str) -> Tuple[Optional[str], Optional[float], str]:
    text = _to_str_or_empty(stem)
    if not text:
    ...
    for sweep_key, pattern in SWEEP_FILENAME_PATTERNS:
        ...
        if sweep_value is None:
            continue
        return sweep_key, sweep_value, f"filename_{sweep_key}"
...
def parse_meta(path: Path) -> FileMeta:
    ...
    sweep_key, sweep_value, sweep_parse = _parse_filename_sweep(path.stem)
    ...
    return FileMeta(
        ...
        sweep_label=_sweep_axis_label(sweep_key) if sweep_key else "",
...
def read_labview_xlsx(meta: FileMeta) -> pd.DataFrame:
    ...
    sweep_key_series = pd.Series(meta.sweep_key or pd.NA, index=df.index, dtype="object")
    sweep_value_series = pd.Series(meta.sweep_value, index=df.index, dtype="float64")
    sweep_label_series = pd.Series(meta.sweep_label or pd.NA, index=df.index, dtype="object")
    ...
    df = df.assign(
        ...
        H2O_pct=h2o_series,
        Sweep_Key=sweep_key_series,
        Sweep_Value=sweep_value_series,
        Sweep_Display_Label=sweep_label_series,
    )
...
def read_motec_csv(meta: FileMeta) -> pd.DataFrame:
    ...
    df["Sweep_Key"] = pd.Series(meta.sweep_key or pd.NA, index=df.index, dtype="object")
    df["Sweep_Value"] = pd.Series(meta.sweep_value, index=df.index, dtype="float64")
    df["Sweep_Display_Label"] = pd.Series(meta.sweep_label or pd.NA, index=df.index, dtype="object")
    ...
    row.update(
        {
            ...
            "Sweep_Key": meta.sweep_key,
            "Sweep_Value": meta.sweep_value,
            "Sweep_Display_Label": meta.sweep_label or pd.NA,
        }
    )
...
def kibox_aggregate(kibox_files: List[FileMeta]) -> pd.DataFrame:
    ...
    if not rows:
        return pd.DataFrame(columns=["Load_kW", "DIES_pct", "BIOD_pct", "EtOH_pct", "H2O_pct", "Sweep_Key", "Sweep_Value"])

    allk = pd.concat(rows, ignore_index=True)
    key_cols = ["SourceFolder", "Load_kW", "DIES_pct", "BIOD_pct", "EtOH_pct", "H2O_pct", "Sweep_Key", "Sweep_Value"]
    ...
    *,
    ...
    if source_mode in {"auto", "text"}:
        if rebuild_text_config or not text_config_exists(text_dir):
            ...
            print(f"[INFO] Gerando config textual compartilhada em: {text_dir}")
...
def _save_runtime_path_settings(
    input_dir: Path,
    out_dir: Path,
    *,
    aggregation_mode: Optional[str] = None,
    sweep_key: Optional[str] = None,
    sweep_x_col: Optional[str] = None,
    sweep_bin_tol: Optional[float] = None,
    helper_configured: Optional[bool] = None,
) -> None:
    RUNTIME_SETTINGS_DIR.mkdir(parents=True, exist_ok=True)
    payload = _load_runtime_path_settings()
    payload.update(
        {
    ...
    if aggregation_mode is not None:
        payload["aggregation_mode"] = _normalize_runtime_aggregation_mode(aggregation_mode)
    if sweep_key is not None:
        payload["sweep_key"] = _normalize_sweep_key(sweep_key)
    if sweep_x_col is not None:
        payload["sweep_x_col"] = _normalize_sweep_x_col(sweep_x_col)
    if sweep_bin_tol is not None:
        payload["sweep_bin_tol"] = _normalize_sweep_bin_tol(sweep_bin_tol)
    if helper_configured is not None:
        payload["helper_configured"] = bool(helper_configured)
...
def _prompt_runtime_dirs_via_tk_dialog(initial_input_dir: Path, initial_out_dir: Path) -> Tuple[Path, Path]:
    if tk is None or ttk is None or filedialog is None or messagebox is None:
        raise RuntimeError(
            f"Tkinter nao esta disponivel neste Python. O {PIPELINE_RUNTIME_NAME} agora exige popup Windows "
            "para selecionar RAW_INPUT_DIR e OUT_DIR."
        )

    root = tk.Tk()
    root.title(f"{PIPELINE_LABEL} - Diretorios de execucao")
    ...
    def confirm() -> None:
        ...
        if not raw_input:
            messagebox.showerror(PIPELINE_LABEL, "Selecione o diretorio de entrada.", parent=root)
            return
        if not out_input:
            messagebox.showerror(PIPELINE_LABEL, "Selecione o diretorio de saida.", parent=root)
            return
        ...
        if not input_dir.exists():
            messagebox.showerror(PIPELINE_LABEL, f"Input dir nao existe:\n{input_dir}", parent=root)
            return
        if not input_dir.is_dir():
            messagebox.showerror(PIPELINE_LABEL, f"Input dir nao e diretorio:\n{input_dir}", parent=root)
            return
        try:
            out_dir.mkdir(parents=True, exist_ok=True)
...
$result = [System.Windows.Forms.MessageBox]::Show(
    'Abrir a GUI de configuracao compartilhada antes de rodar?',
    'Pipeline 30',
...
"""
    ...
    if completed.returncode == 2:
        return None
    raise RuntimeError(
        "Falha ao abrir prompt de GUI do pipeline30. "
...
def _prompt_open_config_gui_via_cli() -> bool:
    raw = input("Abrir GUI de configuracao compartilhada antes de rodar? [y/N]: ").strip().lower()
...
def _choose_runtime_dirs(defaults_cfg: Dict[str, str]) -> Tuple[Path, Path]:
    ...
    use_defaults_env = norm_key(
        os.environ.get("PIPELINE30_USE_DEFAULT_RUNTIME_DIRS", "")
    ...
    if use_defaults_env in {"1", "true", "yes", "on"}:
        print("[INFO] PIPELINE30_USE_DEFAULT_RUNTIME_DIRS ativo; usando RAW_INPUT_DIR/OUT_DIR sem popup.")
    ...
    use_gui_saved_dirs = norm_key(saved_cfg.get("dirs_configured_in_gui", "")) in {"1", "true", "yes", "on"}
    if use_gui_saved_dirs and str(raw_cfg).strip() and str(out_cfg).strip():
...
def _sync_runtime_dirs_to_config_source(
    ...
    if config_bundle.source_kind == "text" and config_bundle.source_path is not None:
        print(
            f"[INFO] Runtime dirs do {PIPELINE_RUNTIME_NAME} ficaram salvos localmente em "
            f"{RUNTIME_SETTINGS_PATH}; a config textual em {config_bundle.source_path} nao foi alterada."
        )


def _planned_pipeline_csv_path(source_open: Path) -> Path:
    planned_name = build_output_name(source_open, name_mode="pipeline", export_type="res")
    return source_open.with_name(planned_name)


def _scan_open_conversion_status(process_dir: Path) -> OpenConversionStatus:
    open_files = sorted(p for p in process_dir.rglob("*.open") if p.is_file())
    missing_csv_opens = [p for p in open_files if not _planned_pipeline_csv_path(p).exists()]
    existing_csv_count = sum(1 for p in open_files if _planned_pipeline_csv_path(p).exists())
...
def _scan_runtime_input_inventory(process_dir: Path) -> RuntimeInputInventory:
    ...
    lv_count = sum(1 for p in candidate_paths if p.suffix.lower() == ".xlsx")
    kibox_csv_count = sum(1 for p in candidate_paths if p.suffix.lower() == ".csv" and p.stem.lower().endswith("_i"))
    motec_csv_count = sum(1 for p in candidate_paths if p.suffix.lower() == ".csv" and p.stem.lower().endswith("_m"))
...
def _available_sweep_keys_from_paths(paths: List[Path]) -> List[str]:
    found: List[str] = []
    for path in paths:
        sweep_key, _sweep_value, _ = _parse_filename_sweep(path.stem)
        if not sweep_key:
            continue
        canonical = _normalize_sweep_key(sweep_key)
...
def _run_missing_open_conversion_batch(status: OpenConversionStatus) -> None:
    missing = list(status.missing_csv_opens)
    if not missing:
        print("[INFO] Conversor .open -> .csv: nenhum arquivo faltante para converter.")
        return

    converter_path = find_opentocsv_exe()
    print(
        f"[INFO] Conversor .open -> .csv: convertendo {len(missing)} arquivo(s) faltante(s) "
        f"com {converter_path}."
    )
    for idx, source_open in enumerate(missing, start=1):
        planned_path = _planned_pipeline_csv_path(source_open)
        print(f"[INFO] [{idx}/{len(missing)}] Convertendo {source_open.name} -> {planned_path.name}")
        result = export_open_file(
            ...
            log_callback=lambda text, is_stderr: print(
                text,
                end="",
        ...
        if result.returncode != 0:
            print(f"[WARN] OpenToCSV retornou codigo {result.returncode} para {source_open.name}.")
    print("[INFO] Conversao .open -> .csv concluida.")


def _prompt_runtime_preflight_via_tk(
    *,
    process_dir: Path,
    lv_count: int,
    kibox_csv_count: int,
    ...
    initial_mode: str,
    initial_sweep_key: str,
    initial_sweep_x_col: str,
) -> Tuple[str, Pipeline30RuntimeSelection]:
    if tk is None or ttk is None or messagebox is None:
        raise RuntimeError("Tkinter nao esta disponivel para o preflight do pipeline30.")

    root = tk.Tk()
    root.title(f"{PIPELINE_LABEL} - preflight")
    root.geometry("860x380")
    root.minsize(760, 320)
    root.columnconfigure(0, weight=1)

    action = {"value": "cancel"}
    mode_var = tk.StringVar(master=root, value=_normalize_runtime_aggregation_mode(initial_mode))
    sweep_var = tk.StringVar(master=root, value=_normalize_sweep_key(initial_sweep_key or available_sweep_keys[0]))
    ...
    summary_lines = [
        ...
        f"LabVIEW: {lv_count} | KiBox CSV: {kibox_csv_count} | MoTeC CSV: {motec_csv_count}",
        f".open: {len(conversion_status.open_files)} | faltando converter: {len(conversion_status.missing_csv_opens)}",
    ]
    ttk.Label(
        frame,
        text="\n".join(summary_lines),
    ...
    ttk.Label(frame, text="Modo do processamento").grid(row=1, column=0, sticky="w", pady=(0, 10))
    ...
    ttk.Label(frame, text="Variavel do sweep").grid(row=2, column=0, sticky="w", pady=(0, 10))
    sweep_display_options = [_sweep_axis_label(key) for key in available_sweep_keys]
    display_to_key = { _sweep_axis_label(key): key for key in available_sweep_keys }
    sweep_display_var = tk.StringVar(master=root, value=_sweep_axis_label(sweep_var.get()))
    ...
    converter_text = (
        "Existem arquivos .open sem CSV pipeline correspondente. "
        "Use o conversor agora para que KiBox e comparacoes nao fiquem faltando."
        if conversion_status.missing_csv_opens
        else "Nao ha .open faltando conversao neste ensaio."
    )
    ...
    def refresh_status(*_args: object) -> None:
        mode = _normalize_runtime_aggregation_mode(mode_var.get())
        sweep_key = display_to_key.get(sweep_display_var.get(), available_sweep_keys[0])
        ...
        if mode == RUNTIME_AGGREGATION_SWEEP:
            status_var.set(
                f"Modo sweep: os plots convencionais que hoje usam carga passarao a usar {_sweep_axis_label(sweep_key)}."
            )
        else:
            status_var.set("Modo load: o fluxo convencional do pipeline continua usando Load_kW.")
    ...
    convert_btn = ttk.Button(button_row, text="Converter .open faltantes", command=convert_now)
    convert_btn.grid(row=0, column=1, padx=(8, 8))
    if not conversion_status.missing_csv_opens:
        convert_btn.configure(state="disabled")

    ttk.Button(button_row, text="Continuar", command=continue_now).grid(row=0, column=2, padx=(8, 8))
    ttk.Button(button_row, text="Cancelar", command=cancel).grid(row=0, column=3, padx=(8, 0))
    ...
    return action["value"], Pipeline30RuntimeSelection(
        aggregation_mode=_normalize_runtime_aggregation_mode(mode_var.get()),
        sweep_key=_normalize_sweep_key(sweep_var.get()),
        sweep_x_col=_normalize_sweep_x_col(initial_sweep_x_col),
    )


def _prompt_runtime_preflight_via_cli(
    *,
    process_dir: Path,
    conversion_status: OpenConversionStatus,
    available_sweep_keys: List[str],
    ...
    print(f"[INFO] {PIPELINE_LABEL} preflight | entrada: {process_dir}")
    print(
        f"[INFO] .open encontrados: {len(conversion_status.open_files)} | "
        f"faltando converter: {len(conversion_status.missing_csv_opens)}"
    )

    mode_raw = input(f"Modo do processamento [load/sweep] ({initial_mode}): ").strip().lower()
    mode = _normalize_runtime_aggregation_mode(mode_raw or initial_mode)

    sweep_key = _normalize_sweep_key(initial_sweep_key)
    if mode == RUNTIME_AGGREGATION_SWEEP:
        options_txt = ", ".join(available_sweep_keys)
        sweep_raw = input(f"Variavel do sweep [{options_txt}] ({sweep_key}): ").strip().lower()
        if sweep_raw:
            sweep_key = _normalize_sweep_key(sweep_raw)

    if conversion_status.missing_csv_opens:
        convert_raw = input("Converter .open faltantes antes de processar? [Y/n]: ").strip().lower()
        if convert_raw not in {"", "y", "yes", "s", "sim"}:
            return "continue", Pipeline30RuntimeSelection(aggregation_mode=mode, sweep_key=sweep_key, sweep_x_col=initial_sweep_x_col)
        return "convert", Pipeline30RuntimeSelection(aggregation_mode=mode, sweep_key=sweep_key, sweep_x_col=initial_sweep_x_col)

    return "continue", Pipeline30RuntimeSelection(aggregation_mode=mode, sweep_key=sweep_key, sweep_x_col=initial_sweep_x_col)


def _choose_runtime_preflight(
    *,
    process_dir: Path,
) -> Pipeline30RuntimeSelection:
    saved_cfg = _load_runtime_path_settings()
    initial_mode = _normalize_runtime_aggregation_mode(saved_cfg.get("aggregation_mode", RUNTIME_AGGREGATION_LOAD))
    inventory = _scan_runtime_input_inventory(process_dir)
    available_sweep_keys = _available_sweep_keys_from_paths(inventory.candidate_paths)
    initial_sweep_key = _normalize_sweep_key(saved_cfg.get("sweep_key", available_sweep_keys[0]))
    initial_sweep_x_col = _normalize_sweep_x_col(saved_cfg.get("sweep_x_col", SWEEP_VALUE_COL))

    while True:
        inventory = _scan_runtime_input_inventory(process_dir)
        available_sweep_keys = _available_sweep_keys_from_paths(inventory.candidate_paths)
        if initial_sweep_key not in available_sweep_keys:
            initial_sweep_key = available_sweep_keys[0]
        conversion_status = _scan_open_conversion_status(process_dir)
        try:
            action, selection = _prompt_runtime_preflight_via_tk(
                process_dir=process_dir,
                lv_count=inventory.lv_count,
                kibox_csv_count=inventory.kibox_csv_count,
                motec_csv_count=inventory.motec_csv_count,
                conversion_status=conversion_status,
                available_sweep_keys=available_sweep_keys,
                initial_mode=initial_mode,
                initial_sweep_key=initial_sweep_key,
                initial_sweep_x_col=initial_sweep_x_col,
            )
        except Exception as exc:
            print(f"[WARN] Preflight GUI do pipeline30 falhou: {exc}. Vou usar fallback no terminal.")
            action, selection = _prompt_runtime_preflight_via_cli(
                process_dir=process_dir,
                conversion_status=conversion_status,
                available_sweep_keys=available_sweep_keys,
                initial_mode=initial_mode,
        ...
        initial_mode = selection.aggregation_mode
        initial_sweep_key = selection.sweep_key
        initial_sweep_x_col = selection.sweep_x_col

        if action == "cancel":
            raise SystemExit("Execucao cancelada pelo usuario no preflight do pipeline30.")
        if action == "convert":
            _run_missing_open_conversion_batch(conversion_status)
            continue
        return selection
...
def _ensure_qt_application() -> Tuple[object, bool]:
    ...
    owns_app = False
    if app is None:
        app = QApplication([PIPELINE_RUNTIME_NAME])
        owns_app = True
...
def _prompt_plot_point_filter_catalog_via_qt(
    ...
    _ = app
    dialog = QDialog()
    dialog.setWindowTitle(f"{PIPELINE_LABEL} - filtro de pontos para plots")
    ...
    def accept_selection() -> None:
        selected = selected_points_now()
        if not selected:
            QMessageBox.critical(dialog, PIPELINE_LABEL, "Selecione pelo menos um ponto para gerar os graficos.")
            return
        _save_plot_point_filter_state(selected, available_points)
...
def _prompt_plot_point_filter_catalog_via_tk(
    ...
    initial_selection, initial_message = _resolve_plot_point_initial_selection(available_points)
    root = tk.Tk()
    root.title(f"{PIPELINE_LABEL} - filtro de pontos para plots")
    root.withdraw()
    ...
    def confirm() -> None:
        selected = {key for key, var in cell_vars.items() if bool(var.get())}
        if not selected:
            messagebox.showerror(PIPELINE_LABEL, "Selecione pelo menos um ponto para gerar os graficos.", parent=root)
            return
        _save_plot_point_filter_state(selected, available_points)
...
def _format_sweep_selector_value(value: object) -> str:
    try:
        val = float(value)
    except Exception:
...
def _cluster_sweep_bin_centers(values: pd.Series, tol: float) -> List[float]:
...
def _candidate_sweep_bin_centers(df: pd.DataFrame, *, x_col: str, tol: float) -> List[float]:
    if df is None or df.empty:
        return []
    ...
    if x_col in df.columns:
        return _cluster_sweep_bin_centers(df[x_col], tol)
    return []


def _assign_value_to_sweep_bin(value: object, *, centers: List[float], tol: float) -> float:
    raw = _to_float(value, default=float("nan"))
    ...
    nearest = min(centers, key=lambda center: abs(raw - center))
    if abs(raw - nearest) <= tol:
        return nearest
    return round(float(raw), 6)


def _apply_runtime_sweep_binning(df: pd.DataFrame) -> pd.DataFrame:
    if df is None:
        return pd.DataFrame()
    out = df.copy()
    if out.empty:
        ...
        return out
    if not _runtime_sweep_active():
        if _runtime_sweep_x_col() in out.columns:
            numeric = pd.to_numeric(out[_runtime_sweep_x_col()], errors="coerce")
            out[SWEEP_BIN_VALUE_COL] = numeric
            out[SWEEP_BIN_LABEL_COL] = numeric.map(_format_sweep_selector_value).replace("", pd.NA)
        return out

    x_col = _runtime_sweep_x_col()
    tol = _runtime_sweep_bin_tol()
    ...
    binned = raw_series.map(lambda value: _assign_value_to_sweep_bin(value, centers=centers, tol=tol))
    out[SWEEP_BIN_VALUE_COL] = pd.to_numeric(binned, errors="coerce")
    out[SWEEP_BIN_LABEL_COL] = out[SWEEP_BIN_VALUE_COL].map(_format_sweep_selector_value).replace("", pd.NA)

    changed_count = int(
        (
            raw_series.round(6).sub(pd.to_numeric(out[SWEEP_BIN_VALUE_COL], errors="coerce").round(6)).abs() > 1e-9
        ).fillna(False).sum()
    )
    if changed_count > 0:
        print(
            f"[INFO] Sweep binning: {changed_count} ponto(s) ajustado(s) para bins com tolerancia +/-{tol:.4f} "
            f"usando {len(centers)} centro(s)."
        )
    else:
        print(f"[INFO] Sweep binning: nenhum ajuste necessario; tolerancia +/-{tol:.4f}, centros={len(centers)}.")
    return out


def _build_sweep_duplicate_catalog(
    df: pd.DataFrame,
    *,
    x_col: str,
) -> Tuple[List[str], List[float], Dict[Tuple[str, float], List[str]]]:
    if df is None or df.empty or "BaseName" not in df.columns or x_col not in df.columns:
        return [], [], {}

    labels = _plot_point_fuel_labels(df)
    x_values = pd.to_numeric(df.get(x_col, pd.Series(pd.NA, index=df.index)), errors="coerce").round(4)
    basenames = df.get("BaseName", pd.Series(pd.NA, index=df.index, dtype="object")).map(_to_str_or_empty)
    tmp = pd.DataFrame(
        {
            "Fuel_Label": labels,
    ...
    cell_map: Dict[Tuple[str, float], List[str]] = {}
    for (fuel_label, sweep_value), group in tmp.groupby(["Fuel_Label", "Sweep_Value_Runtime"], dropna=False, sort=True):
        names = sorted(dict.fromkeys(group["BaseName"].astype(str).tolist()), key=_canon_name)
        cell_map[(str(fuel_label), float(sweep_value))] = names

    fuel_labels = _preferred_fuel_label_order([fuel for fuel, _value in cell_map.keys()])
    sweep_values = sorted({float(value) for _fuel, value in cell_map.keys()})
    return fuel_labels, sweep_values, cell_map


class _SweepDuplicateSelectorDialog(QDialog if QDialog is not None else object):
    def __init__(
        self,
        *,
        fuel_labels: List[str],
        ...
        self.setWindowTitle("Pipeline 30 - seletor de ensaios por combustivel x varredura")
        self.resize(1280, 780)
        self.fuel_labels = fuel_labels
        self.sweep_values = sweep_values
        self.cell_map = cell_map
        self.axis_label = axis_label
        self._file_checkboxes: Dict[Tuple[str, float], List[Tuple[str, QCheckBox]]] = {}
        self._master_checkboxes: Dict[Tuple[str, float], QCheckBox] = {}
        self._syncing_keys: set[Tuple[str, float]] = set()

        root = QVBoxLayout(self)
        intro = QLabel(
            "Colunas = valores da varredura. Linhas = combustiveis. "
            "Cada celula lista os arquivos do ponto. Em duplicatas, o checkbox principal marca todos os arquivos "
            "daquele lambda para o combustivel; os checkboxes ao lado de cada nome deixam voce escolher arquivo por arquivo."
        )
        ...
        self.table.setHorizontalHeaderLabels([_format_sweep_selector_value(value) for value in sweep_values])
        ...
        for row, fuel_label in enumerate(self.fuel_labels):
            for col, sweep_value in enumerate(self.sweep_values):
                ...
                if not available:
                    ...
                    continue
                self.table.setCellWidget(row, col, self._build_cell_widget(key, available))
        self.table.resizeRowsToContents()
        self._refresh_status()

    def _build_cell_widget(self, key: Tuple[str, float], available: List[str]) -> QWidget:
        ...
        if len(available) > 1:
            master_checkbox = QCheckBox(f"Todos ({len(available)}/{len(available)})", cell_widget)
            master_checkbox.setTristate(True)
            master_checkbox.stateChanged.connect(
                lambda state, _key=key: self._handle_master_checkbox_change(_key, state)
            )
            cell_layout.addWidget(master_checkbox)
            self._master_checkboxes[key] = master_checkbox

        children: List[Tuple[str, QCheckBox]] = []
        for basename in available:
            row_widget = QWidget(cell_widget)
            ...
            checkbox.stateChanged.connect(lambda _state, _key=key: self._handle_child_checkbox_change(_key))

            label = QLabel(basename, row_widget)
            label.setWordWrap(True)
            label.setToolTip(basename)
            label.setStyleSheet("color: #1f2933;")

            row_layout.addWidget(checkbox, alignment=Qt.AlignTop)
            row_layout.addWidget(label, 1)
            cell_layout.addWidget(row_widget)
            children.append((basename, checkbox))

        self._file_checkboxes[key] = children
        self._refresh_master_checkbox(key)
        return cell_widget

    def _set_key_syncing(self, key: Tuple[str, float], syncing: bool) -> None:
        if syncing:
            self._syncing_keys.add(key)
        else:
            self._syncing_keys.discard(key)

    def _selected_basenames_for_key(self, key: Tuple[str, float]) -> List[str]:
        return [
            basename
            for basename, checkbox in self._file_checkboxes.get(key, [])
            if bool(checkbox.isChecked())
        ]

    def _refresh_master_checkbox(self, key: Tuple[str, float]) -> None:
        master = self._master_checkboxes.get(key)
        if master is None:
            return
        children = self._file_checkboxes.get(key, [])
        selected_count = sum(1 for _basename, checkbox in children if bool(checkbox.isChecked()))
        total_count = len(children)
        self._set_key_syncing(key, True)
        try:
            master.setText(f"Todos ({selected_count}/{total_count})")
            if selected_count <= 0:
            ...
            self._set_key_syncing(key, False)

    def _handle_master_checkbox_change(self, key: Tuple[str, float], state: int) -> None:
        if key in self._syncing_keys:
            return
        if state not in {int(Qt.Checked), int(Qt.Unchecked)}:
            return
        new_value = state == int(Qt.Checked)
        self._set_key_syncing(key, True)
        try:
            for _basename, checkbox in self._file_checkboxes.get(key, []):
    ...
    def _handle_child_checkbox_change(self, key: Tuple[str, float]) -> None:
        if key in self._syncing_keys:
            return
        self._refresh_master_checkbox(key)
        self._refresh_status()
    ...
    def _refresh_status(self) -> None:
        total_points = len(self.cell_map)
        selected_points = 0
        selected_files = 0
        duplicate_points = sum(1 for values in self.cell_map.values() if len(values) > 1)
        for key in self.cell_map:
            selected_here = self._selected_basenames_for_key(key)
            if selected_here:
                selected_points += 1
                selected_files += len(selected_here)
        self.status_label.setText(
            f"Pontos selecionados: {selected_points} / {total_points} | "
            f"Arquivos marcados: {selected_files} | Duplicatas: {duplicate_points}"
        )
    ...
    def _select_all(self) -> None:
        for key, children in self._file_checkboxes.items():
            self._set_key_syncing(key, True)
            try:
                for _basename, checkbox in children:
                    checkbox.setChecked(True)
            finally:
                self._set_key_syncing(key, False)
            self._refresh_master_checkbox(key)
    ...
    def _keep_first_per_duplicate(self) -> None:
        for key, children in self._file_checkboxes.items():
            self._set_key_syncing(key, True)
            try:
                for idx, (_basename, checkbox) in enumerate(children):
                    checkbox.setChecked(len(children) == 1 or idx == 0)
            finally:
                self._set_key_syncing(key, False)
            self._refresh_master_checkbox(key)
        self._refresh_status()
    ...
    def selected_basenames(self) -> set[str]:
        ...
        for key in self.cell_map:
            selected.update(str(name).strip() for name in self._selected_basenames_for_key(key) if str(name).strip())
        return selected


def prompt_sweep_duplicate_filter(df: pd.DataFrame, *, x_col: str) -> Optional[set[str]]:
    fuel_labels, sweep_values, cell_map = _build_sweep_duplicate_catalog(df, x_col=x_col)
    if not cell_map:
        return None

    duplicate_cells = {key: values for key, values in cell_map.items() if len(values) > 1}
    if not duplicate_cells:
        print("[INFO] Sweep selector: nao encontrei duplicatas por combustivel x varredura.")
        return None

    axis_label = _runtime_sweep_axis_label_for_col(x_col)
    try:
        app, owns_app = _ensure_qt_application()
    except Exception as exc:
        print(f"[WARN] GUI de duplicatas sweep indisponivel: {exc}. Vou manter todos os ensaios.")
        return None
    ...
    try:
        ...
        if not selected:
            print("[WARN] Sweep selector retornou selecao vazia; vou manter todos os ensaios.")
            return None
        return selected
    finally:
        if owns_app:
...
def _apply_sweep_duplicate_filter(df: pd.DataFrame, selected_basenames: Optional[set[str]]) -> pd.DataFrame:
    ...
    print(f"[INFO] Sweep selector: mantive {len(out)}/{len(df)} ensaio(s) apos o filtro de duplicatas.")
    return out
...
def compute_trechos_stats(lv_raw: pd.DataFrame, instruments_df: pd.DataFrame) -> pd.DataFrame:
    ...
    group_cols = ["BaseName", "Load_kW", "DIES_pct", "BIOD_pct", "EtOH_pct", "H2O_pct", "Sweep_Key", "Sweep_Value", "WindowID"]
...
def compute_ponto_stats(trechos: pd.DataFrame) -> pd.DataFrame:
    ...
    group_cols = ["BaseName", "Load_kW", "DIES_pct", "BIOD_pct", "EtOH_pct", "H2O_pct", "Sweep_Key", "Sweep_Value"]
...
def compute_motec_ponto_stats(motec_trechos: pd.DataFrame) -> pd.DataFrame:
    ...
    group_cols = ["Load_kW", "DIES_pct", "BIOD_pct", "EtOH_pct", "H2O_pct", "Sweep_Key", "Sweep_Value"]
...
def build_final_table(
    ...
    if kibox_agg is not None and not kibox_agg.empty:
        df = _left_merge_on_fuel_keys(df, kibox_agg, extra_on=["SourceFolder", "Load_kW", "Sweep_Key", "Sweep_Value"])
    if motec_ponto is not None and not motec_ponto.empty:
        df = _left_merge_on_fuel_keys(df, motec_ponto, extra_on=["Load_kW", "Sweep_Key", "Sweep_Value"])
...
def _runtime_sweep_axis_label_for_col(x_col: object) -> str:
    col = _to_str_or_empty(x_col)
    col_norm = norm_key(col) if col else ""
    if not col or col_norm in {norm_key(SWEEP_VALUE_COL), norm_key(SWEEP_BIN_VALUE_COL)}:
        source_col = _runtime_sweep_x_col()
        source_norm = norm_key(source_col)
        if source_col and source_norm not in {norm_key(SWEEP_VALUE_COL), norm_key(SWEEP_BIN_VALUE_COL)}:
        ...
        return _sweep_axis_label(_runtime_sweep_key())
    pretty = re.sub(r"(?i)_mean_of_windows$", "", col)
    pretty = re.sub(r"(?i)_sd_of_windows$", "", pretty)
    return pretty if pretty else col


def _runtime_sweep_axis_token_for_col(x_col: object) -> str:
    label = _runtime_sweep_axis_label_for_col(x_col)
    token = _safe_name(label)
    if token:
        return token
    col = _to_str_or_empty(x_col)
    return _safe_name(col) if col else _safe_name(_runtime_sweep_key())
...
def _resolve_plot_x_request(x_col_req: str) -> Tuple[str, bool]:
    ...
    if _runtime_sweep_matches_load_request(req):
        return _runtime_sweep_effective_x_col(), True
    if _runtime_sweep_active() and req_norm in {
        norm_key(_runtime_sweep_x_col()),
        norm_key(SWEEP_BIN_VALUE_COL),
    }:
        return _runtime_sweep_effective_x_col(), True
...
def _runtime_plot_x_label(
    ...
    if _runtime_sweep_active() and norm_key(x_col_resolved) in {
        norm_key(_runtime_sweep_x_col()),
        norm_key(_runtime_sweep_effective_x_col()),
    }:
        label_norm = norm_key(label) if label else ""
        x_base_norm = norm_key(x_col_base)
        if not label or label_norm in {
            x_base_norm,
            norm_key(x_col_resolved),
            norm_key("Load_kW"),
            norm_key("Carga (kW)"),
            norm_key("Power (kW)"),
            norm_key("Power"),
            norm_key("Potencia (kW)"),
            norm_key(SWEEP_VALUE_COL),
            norm_key(SWEEP_BIN_VALUE_COL),
        }:
            return _runtime_sweep_axis_label_for_col(_runtime_sweep_x_col())
...
def _runtime_plot_fixed_x(
    x_col_req: str,
    fixed_x: Optional[Tuple[float, float, float]],
) -> Optional[Tuple[float, float, float]]:
    if _runtime_sweep_matches_load_request(x_col_req):
        return None
    if _runtime_sweep_active() and norm_key(x_col_req) in {
        norm_key(_runtime_sweep_x_col()),
        norm_key(SWEEP_BIN_VALUE_COL),
    }:
        return None
...
def _runtime_plot_filename_title(
    filename: str,
    title: str,
    *,
    x_col_req: str,
    ...
    if not (
        _runtime_sweep_active()
        and norm_key(x_col_resolved) in {norm_key(_runtime_sweep_x_col()), norm_key(_runtime_sweep_effective_x_col())}
        and (
            _runtime_sweep_matches_load_request(x_col_req)
            or norm_key(x_col_req) in {norm_key(_runtime_sweep_x_col()), norm_key(SWEEP_BIN_VALUE_COL)}
        )
    ):
        return filename, title

    sweep_token = _runtime_sweep_axis_token_for_col(_runtime_sweep_x_col())
    sweep_label = _runtime_sweep_axis_label_for_col(_runtime_sweep_x_col())

    out_filename = filename
    out_title = title
    if out_filename:
        out_filename = re.sub(r"(?i)vs_power", f"vs_{sweep_token}", out_filename)
        out_filename = re.sub(r"(?i)vs_load", f"vs_{sweep_token}", out_filename)
        out_filename = re.sub(r"(?i)load_k_w", sweep_token, out_filename)
        out_filename = re.sub(r"(?i)load_kw", sweep_token, out_filename)
    if out_title:
        out_title = re.sub(r"(?i)\bvs load\b", f"vs {sweep_label}", out_title)
        out_title = re.sub(r"(?i)\bvs power\b", f"vs {sweep_label}", out_title)
        out_title = re.sub(r"(?i)\bload_kW\b", sweep_label, out_title)
        out_title = re.sub(r"(?i)\bload\b", sweep_label, out_title)
    return out_filename, out_title
...
def make_plots_from_config(
    ...
    for _, r in plots_df.iterrows():
        ...
        if pt in {"kibox_all", "all_kibox"}:
            ...
            fixed_x_runtime = _runtime_plot_fixed_x(x_col_req, fixed_x)
            try:
            ...
            for yc in sorted(kibox_cols):
                ...
                fn, tt = _runtime_plot_filename_title(fn, tt, x_col_req=x_col_req, x_col_resolved=x_col)
        ...
        if pt in {"all_fuels_yx", "all_fuels", "all_fuels_y_vs_x"}:
            ...
            filename, title = _runtime_plot_filename_title(filename, title, x_col_req=x_col_req, x_col_resolved=x_col)
...
def _parse_cli_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Pipeline 30 com configuracao textual compartilhada e fallback para Excel.")
    parser.add_argument("--config-source", choices=["auto", "text", "excel"], default="auto")
    parser.add_argument("--config-dir", default="", help="Diretorio da configuracao textual compartilhada.")
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    print(f"[INFO] {PIPELINE_LABEL} | Base do script: {BASE_DIR}")
    ...
    if args.config_gui:
        try:
            ...
            raise RuntimeError(f"Nao consegui abrir a GUI de configuracao compartilhada: {exc}") from exc
    ...
    skip_gui_prompt = norm_key(
        os.environ.get("PIPELINE30_SKIP_CONFIG_GUI_PROMPT", "")
        or os.environ.get("PIPELINE29_SKIP_CONFIG_GUI_PROMPT", "")
    )
    if not args.skip_config_gui_prompt and skip_gui_prompt not in {"1", "true", "yes", "on"}:
        if _prompt_open_config_gui():
            try:
                ...
                print(f"[WARN] Nao consegui abrir a GUI de configuracao compartilhada: {exc}")
            else:
                gui_save_run_exit_code = PIPELINE29_GUI_SAVE_RUN_EXIT_CODE
    ...
    saved_runtime_cfg = _load_runtime_path_settings()
    helper_configured = norm_key(saved_runtime_cfg.get("helper_configured", "")) in {"1", "true", "yes", "on"}
    if helper_configured:
        runtime_selection = Pipeline30RuntimeSelection(
            aggregation_mode=_normalize_runtime_aggregation_mode(saved_runtime_cfg.get("aggregation_mode", RUNTIME_AGGREGATION_LOAD)),
            sweep_key=_normalize_sweep_key(saved_runtime_cfg.get("sweep_key", DEFAULT_SWEEP_KEY)),
            sweep_x_col=_normalize_sweep_x_col(saved_runtime_cfg.get("sweep_x_col", SWEEP_VALUE_COL)),
            sweep_bin_tol=_normalize_sweep_bin_tol(saved_runtime_cfg.get("sweep_bin_tol", DEFAULT_SWEEP_BIN_TOL)),
        )
        print("[INFO] Usando modo sweep/load salvo pela GUI compartilhada.")
    else:
        runtime_selection = _choose_runtime_preflight(process_dir=PROCESS_DIR)
    _runtime_set_plot_mode(runtime_selection)
    _save_runtime_path_settings(
        PROCESS_DIR,
        OUT_DIR,
        aggregation_mode=runtime_selection.aggregation_mode,
        sweep_key=runtime_selection.sweep_key,
        sweep_x_col=runtime_selection.sweep_x_col,
        sweep_bin_tol=runtime_selection.sweep_bin_tol,
    )
    print(
        f"[INFO] Modo de processamento: {runtime_selection.aggregation_mode}"
        + (
            f" | eixo sweep: {_sweep_axis_label(runtime_selection.sweep_key)} | bin tol: +/-{runtime_selection.sweep_bin_tol:.4f}"
            if _runtime_sweep_active()
            else ""
        )
    )
    ...
    runtime_inventory = _scan_runtime_input_inventory(PROCESS_DIR)
    raw_files = [p for p in runtime_inventory.candidate_paths if p.suffix.lower() in {".xlsx", ".csv"}]
    ...
    kibox_agg = (
        ...
        if kibox_files
        else pd.DataFrame(columns=["Load_kW", "DIES_pct", "BIOD_pct", "EtOH_pct", "H2O_pct", "Sweep_Key", "Sweep_Value"])
    )
    motec_ponto = pd.DataFrame(columns=["Load_kW", "DIES_pct", "BIOD_pct", "EtOH_pct", "H2O_pct", "Sweep_Key", "Sweep_Value"])
    if motec_files:
    ...
    if _runtime_sweep_active():
        out = _apply_runtime_sweep_binning(out)
        sweep_x_col = _runtime_sweep_effective_x_col()
        if sweep_x_col not in out.columns:
            print(
                f"[WARN] Modo sweep: coluna de varredura binada '{sweep_x_col}' nao apareceu no output agregado. "
                "Vou manter todos os ensaios e os plots poderao ser pulados se essa coluna continuar ausente."
            )
        else:
            selected_sweep_basenames = prompt_sweep_duplicate_filter(out, x_col=sweep_x_col)
            out = _apply_sweep_duplicate_filter(out, selected_sweep_basenames)

    out_xlsx = safe_to_excel(out, OUT_DIR / "lv_kpis_clean.xlsx")
    print(f"[OK] Excel gerado: {out_xlsx}")
    if selected_plot_points is None and not _runtime_sweep_active():
    ...
    for source_folder, plot_dir, out_group in iter_source_plot_groups(plot_out):
        ...
        if _runtime_sweep_active():
            print("[INFO] Modo sweep ativo: pulei overlays e suites especiais que dependem de carga nominal.")
            continue
        _merge_plot_run_summary(
            plot_summary_total,
    ...
    compare_groups = [] if _runtime_sweep_active() else iter_compare_plot_groups(plot_out, root=PLOTS_DIR)
    if compare_groups:
        ...
        if _runtime_sweep_active():
            print("[INFO] Modo sweep ativo: pulei os plots de compare/ e de baseline vs aditivado.")
        else:
